...
#define number_of_vaccine 200 // For the number of vaccines already in stock to begin with
...
int main()
{
    ...
    cout << "\n\n\t\t\t\xB3\xB2 \xB2 \xB2 \xB3 VACCINE MANAGEMENT SYSTEM  \xB3 \xB2 \xB2 \xB2\xB3\n\n"
         << endl;
    cout << "\t\t\t\t\t\t\t   Presented By:- Ayush Kamal " << endl
         << endl;
...
int admin_login()
{
    ...
    if ((strcmp(admin_username, "ayushkamal01") == 0) && (strcmp(admin_password, "DolbyAtmos") == 0) && (captcha_input == captcha))
    {
        return 1;
    }
...
void staff_login_page()
{
    ...
    {
        system("cls");
        cout << "\n\n\n\n\t\t\t\tThanks for using this program :) \n\t\t\t\tThis program is created by Ayush Kamal.\n\n";
        break;
    }
    default:
    {
...
void vaccine ::view_vaccine()
{
    ...
    { // code to read the records one at a time
        number_of_records++;
    }
    ind.close();
...
void vaccine::search_by_covid_history()
{
    ...
    while (!in.eof())
    {
        if (toupper(search_covid_history) == toupper(covid_patient))
        { /*Avoiding making cases seperatety by coverting to uppercase before comparision */
            show_list();
            flag = 1;
            number_of_records_found++;
        }
